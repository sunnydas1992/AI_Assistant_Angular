...
- **Knowledge base**: Populate from Jira tickets, Confluence URLs, and uploaded files (PDF, DOCX, TXT, MD). Sources list is fully visible in both light and dark mode.
- **Ticket analyzer**: Load tickets and chat with AI (quick actions: summarize, find gaps, risk analysis, test suggestions). AI model defaults to the one selected in Configuration. Each assistant response has a **Copy** button to copy content to the clipboard and a **Post to Jira** button that opens a preview overlay where the comment can be reviewed and edited before confirming. Ticket detail overlays render **Jira wiki markup as formatted HTML** (bold, italic, underline, links, headings, lists, etc.) using Jira's rendered fields with a fallback wiki-to-HTML converter. **Large file attachments** (PDFs, docs > 30K chars) are automatically chunked, embedded, and indexed in an in-memory vector store — each question retrieves only the most relevant sections, so even 100+ page documents are fully searchable without truncation. An upload progress overlay provides visual feedback during file processing. **View Similar Tickets** — from the ticket detail overlay, find previously resolved tickets similar to the current one using **semantic similarity** (sentence-transformer embeddings + cosine ranking). Results include similarity score, status, resolution, assignee, and comment count, with **Open in Jira** links. Users can select/deselect or remove irrelevant tickets, then **Generate AI Summary** to get a Bedrock-powered analysis of common resolution patterns, specific solutions, and key takeaways for the selected tickets only. Selected similar tickets can also be **added to the chat context** so the AI can reference them during conversation.
- **Test cases**: Generate from a Jira ticket or Confluence page (BDD Gherkin or Xray format). You can provide **optional instructions** before generating (e.g. consider accessibility tests, performance, edge cases). Optional RAG; **refine all** test cases with global feedback or **refine a single** test case with per-item feedback; **AI confidence scores** (1–5) per test case with a **human-in-the-loop guardrail**: cases with confidence below 3 are flagged "Needs Review" and must be explicitly approved by the user before they can be published; **Check Xray for duplicates** — two-pass detection: (1) exact normalized match, (2) **semantic similarity** via sentence embeddings (cosine similarity ≥ 80%) with match type and similarity score badges; publish skips duplicate summaries by default; edit and select which to publish; write selected or all test cases to **Jira Xray**; export to Excel. Ticket detail overlays render Jira wiki markup as formatted HTML. Long-running actions show a shared **thinking** overlay (progress steps) across Test Cases, Test Plan, Ticket Analyzer, Knowledge Base, Configuration, and Home.
...
│       ├── services/     # JiraRAG, chat, vector store, Atlassian, xray_duplicate, similar tickets, etc.
